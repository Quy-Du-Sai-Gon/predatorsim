...
public class HuntingInGroup extends Hunting {
    ...
    // getComponent(Predator.class).orElseThrow().animalStat;
    ...
    // private void howl() {
    // if (groupVision.getAllDetectedObject(Predator.class).size() >=
    // numberOfAllies) {
    // groupFounded = true;
    // return;
    // }

    // var predatorsInRange =
    // howlVision.getAllDetectedWanderingObject(Predator.class);
    // if (!predatorsInRange.isEmpty()) {
    // alliesObjects = predatorsInRange.stream().limit(numberOfAllies).toList();

    // for (var ally : alliesObjects) {
    // Predator animal = (Predator) GameObject.getComponent(ally,
    // Animal.class).get();
    ...
    // private void join() {
    // if (Distance.calculateDistance(leaderObject, getGameObject()) <=
    // predatorStat.groupRadius) {
    // if (GameObject.getComponent(leaderObject,
    // HuntingInGroup.class).get().groupFounded) {
    // groupFounded = true;
    // return;
    // }
    // stalk();
    ...
    // Point2D targetDir;
    // Component component = GameObject.getComponent(leaderObject,
    // Component.class).get();
    ...
    // Animal animal = GameObject.getComponent(targetObject,
    // Animal.class).orElseThrow();
    // WanderBehaviour targetMovement =
    // animal.getComponent(WanderBehaviour.class).orElseThrow();

    // double targetX, targetY;
    // Point2D targetDir;
    // Component component = GameObject.getComponent(targetObject,
    ...
    // velocity.set(
    // (targetMovement.noiseX + targetDir.getX()) * animalStat.runSpeed *
    // Time.getDeltaTime()
    // * Parameter.getRelativeSimulationSpeed(),
    // (targetMovement.noiseY + targetDir.getY()) * animalStat.runSpeed *
    // Time.getDeltaTime()
    // * Parameter.getRelativeSimulationSpeed());
    // }
    // }
    ...
    // private void stalk() {
    // double distanceToTarget = Distance.calculateDistance(getGameObject(),
    // targetObject);
    // double targetX, targetY;
    // Point2D targetDir;
    ...
    // ((PredatorStat) allyAnimal.animalStat).starvationResilience += nutrition /
    // (numberOfAllies + 1);
    ...
    // =============================================================");
    // GameObject.getComponent(leaderObject,
    // HuntingInGroup.class).get().divideFood(nutrition);
    // }
