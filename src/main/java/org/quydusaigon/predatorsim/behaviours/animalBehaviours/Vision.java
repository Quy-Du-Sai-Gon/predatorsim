...
public class Vision extends Behaviour {
    ...
    // Add a new game object to the set of detected game object of the vision instance of a game object if the collider 
    ...
    public void onCollisionEnter(Collider<?> collider, Collider<?> other) {
        ...
        // Check if the other game object is also not the game object of this collider and 
        ...
        if (thisAnimalGameObject != otherGameObject
            ...
            // Add the other game object to the set of all detected game object of this vision instance
            detectedGameObject.add(otherGameObject);
        }
    }

    // Remove a game object from the set of detected game object of the vision instance of a game object if the collider 
    // of the vision no longer collides with the game object
    @Override
    public void onCollisionExit(Collider<?> collider, Collider<?> other) {
        // Check if the game object of the other collider is not the same game object as this collider
        if (GameObject.getParent(getGameObject()).get() != other.getGameObject()) {
            // Remove the game object of the other collider from the set of all detected game object of this vision instance
            detectedGameObject.remove(other.getGameObject());
        }
    }

    /**
     * Return a {@link Set} of {@link Group} containing an {@link Animal} instance of type {@code animal} that is currently 
     ...
     * @return a {@link Set} of {@link Group} colliding with this vision instance that containing the instance of type {@code animal}.
     */
    ...
    /**
     * Return the closest {@link Group} containing an {@link Animal} instance of type {@code animalClass} and its {@link Animal}'s
     * {@code currentState} instance is of type {@code stateClass} (if any), that is colliding with this vision instance
     * @param <T> the type of {@link Animal} to return.
     ...
     * @return an {@link Optional} containing the closest {@link Group} that contains an {@link Animal} 
     * instance of type {@code animalClass} and its {@link Animal}'s {@code currentState} instance is of type {@code stateClass}.
     */
    ...
    /**
     * Return the closest {@link Group} containing an {@link Animal} instance of type {@code animal}
     * @param <T> the type of {@link Animal} to return.
